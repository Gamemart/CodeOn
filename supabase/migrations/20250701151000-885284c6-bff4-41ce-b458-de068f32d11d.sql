...
-- Drop any existing policies from previous migrations
...
DROP POLICY IF EXISTS "chat_participants_insert_self_policy" ON public.chat_participants;
DROP POLICY IF EXISTS "chat_participants_insert_creator_policy" ON public.chat_participants;
...
-- Drop the policies that are causing conflicts
...
DROP POLICY IF EXISTS "Users can join chats themselves" ON public.chat_participants;
...
DROP POLICY IF EXISTS "Users can send messages to accessible chats" ON public.messages;
...
DROP FUNCTION IF EXISTS public.user_can_access_chat(uuid);
